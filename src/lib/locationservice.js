export default class LocationService {
    ...
    async allLocations() {
        const response = await this.fetch('/api/location?sort_field=id');
        const pageRequests = Array(response.meta.pages).fill(1).map((i, j) => j + 1)
        pageRequests.shift();
        const rest = await Promise.all((pageRequests || []).map(page => this.fetch(`/api/location?sort_field=id&page=${page}`)));
